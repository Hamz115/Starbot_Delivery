...
#             transform.header.frame_id = 'D415_color_frame'  # Updated frame ID
...
#             pose_array.header.frame_id = 'D415_color_frame'  # Updated frame ID
...
#                     depth = float(depth_image[center_y, center_x]) / 1000.0  # Convert from millimeters to meters / 1000.0  # Convert mm to meters
#                     if np.isnan(depth):
#                         continue
...
#                     # Calculate 3D position using camera matrix and intrinsics directly
#                     X = (center_x - self.camera_params['cx']) * depth / self.camera_params['fx']
#                     Y = (center_y - self.camera_params['cy']) * depth / self.camera_params['fy']
#                     Z = depth
...
class HoleDetectorReal(Node):
    def __init__(self):
        ...
        self.camera_params = {
            ...
            'cy': 124.9103012084961,   # optical center y
            'hole_depth': 0.040,       # meters
            'safety_offset': 0.015     # Additional safety offset in meters
        }
        ...
        try:
            ...
            for result in results:
                ...
                for box in boxes:
                    ...
                    depth = float(depth_image[center_y, center_x]) / 1000.0  # Convert mm to meters
                    if np.isnan(depth):
                        continue
            ...
            if len(holes) == 4:
                ...
                for i, hole in enumerate(ordered_holes, 1):
                    ...
                    # Calculate 3D position using OpenCV methods
                    ...
                    # Adjust for hole depth with safety offset
                    adj_x, adj_y, adj_z = self._adjust_for_hole_depth(
                        float(point_3d[0]), float(point_3d[1]), float(point_3d[2])
                    )
